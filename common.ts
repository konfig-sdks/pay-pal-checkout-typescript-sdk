...
import { readableStreamToString, PayPalCheckoutError, parseIfJson } from "./error";
    ...
    const authenticate = async () => {
        if (configuration.oauthClientId && configuration.oauthClientSecret && configuration.accessToken === undefined) {
            const token = await wrapAxiosRequest(async () => {
                ...
                const url = configuration.oauthTokenUrl ?? removeTrailingSlash(configuration.basePath) + "/v1/oauth2/token"
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new PayPalCheckoutError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new PayPalCheckoutError(e, e.response?.data)
                }
                if (innerError instanceof PayPalCheckoutError) {
                    // Got "PayPalCheckoutError" from the above try block
                    throw innerError;
                }
